        ...
        <div class="md:w-1/3 flex md:justify-end items-center gap-3">
            ...
            <div class="relative z-30 md:mt-0 mt-6">
                <button id="dropdownButton" 
                    ...
                    Filter <i class="fas fa-ellipsis-v ml-2"></i>
                </button>
                <div id="dropdownMenu" 
                    class="absolute right-0 mt-2 w-40 bg-gray-800 text-gray-200 rounded-md shadow-lg hidden">
                    
                    {{-- Link untuk menghapus filter sort --}}
                    <a href="{{ route('film', ['genre' => request('genre'), 'search' => request('search')]) }}" 
                    class="block px-4 py-2 hover:bg-gray-700 @if(!request('sort')) bg-gray-700 @endif">
                        Default
                    </a>

                    <a href="{{ route('film', ['genre' => request('genre'), 'search' => request('search'), 'sort' => 'popular']) }}" 
                    class="block px-4 py-2 hover:bg-gray-700 @if(request('sort') == 'popular') bg-gray-700 @endif">
                        Terpopuler
                    </a>
                    
                    <a href="{{ route('film', ['genre' => request('genre'), 'search' => request('search'), 'sort' => 'latest']) }}" 
                    class="block px-4 py-2 hover:bg-gray-700 @if(request('sort') == 'latest') bg-gray-700 @endif">
                        Terbaru
                    </a>

                    <a href="{{ route('film', ['genre' => request('genre'), 'search' => request('search'), 'sort' => 'oldest']) }}" 
                    class="block px-4 py-2 hover:bg-gray-700 @if(request('sort') == 'oldest') bg-gray-700 @endif">
                        Terlawas
                    </a>
