...
Route::get('login', function () {
    // Ambil client ID dari file config, yang membaca dari .env
    $googleClientId = config('services.google.client_id');
    return view('auth.login', ['googleClientId' => $googleClientId]);
})->name('login');

Route::get('/google-callback-page', function (Illuminate\Http\Request $request) {
    if (!$request->has('code')) {
        return redirect('/login')->with('error', 'Login dengan Google gagal.');
    }
    return view('auth.google-callback', ['code' => $request->query('code')]);
});
